    ...
    if (window != priv->dnd_proxy_win) {
        if (window != widget->window && window != NULL && window != widget->window) {
            ...
            if (GDK_WINDOW_XID(window) != target) {
                ...
                if (window == NULL) {
                    window = gdk_window_foreign_new(target);
                }
            }
        ...
        MouseCheckType check_type)
{
    ...
    case MOUSE_CHECK_EDGE_ONLY: {
        ...
        if (mouse_screen_num != panel_screen_num) {
            return FALSE;
        }
...
awn_panel_schedule_redraw(gpointer user_data)
{
    ...
    case 0:
        priv->withdraw_redraw_timer = 50;
        break;
    case 50:
        priv->withdraw_redraw_timer = 150;
        break;
    case 150:
        priv->withdraw_redraw_timer = 350;
        break;
    case 350:
        priv->withdraw_redraw_timer = 850;
        break;
    default:
        priv->withdraw_redraw_timer = 0;
        break;
    }
...
awn_panel_uninhibit_autohide(AwnPanel* panel, guint cookie)
{
    ...
    if (!item) {
        return TRUE;    // we could set an error
    }
