...
awn_background_finalize(GObject* object)
{
    ...
    if (bg->g_step_1) {
        g_object_unref(bg->g_step_1);
    }
    if (bg->g_step_2) {
        g_object_unref(bg->g_step_2);
    }
    if (bg->g_histep_1) {
        g_object_unref(bg->g_histep_1);
    }
    if (bg->g_histep_2) {
        g_object_unref(bg->g_histep_2);
    }
    if (bg->border_color) {
        g_object_unref(bg->border_color);
    }
    if (bg->hilight_color) {
        g_object_unref(bg->hilight_color);
    }
    if (bg->sep_color) {
        g_object_unref(bg->sep_color);
    }
                                 ...
                                 gint* strut_start, gint* strut_end)
{
    ...
    klass = AWN_BACKGROUND_GET_CLASS(bg);
    if (klass->get_strut_offsets == NULL) {
        return;
    }
