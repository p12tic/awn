...
_on_configure_event(AwnTooltip* tooltip, GdkEventConfigure* event)
{
    ...
    priv->old_w = event->width;
    priv->old_h = event->height;
                 ...
                 GtkWidget* focus)
{
    ...
    if (gtk_widget_get_visible(GTK_WIDGET(tooltip))) {
        return FALSE;
    }
...
awn_tooltip_hide_timer(gpointer data)
{
    ...
    if (!tooltip->priv->smart_behavior) {
        return FALSE;
    }
                 ...
                 GtkWidget* focus)
{
    ...
    if (priv->inhibit_show) {
        priv->inhibit_show = FALSE;
    }

    if (!priv->smart_behavior) {
        return FALSE;
    }
