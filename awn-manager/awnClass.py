...
class awnApplet(awnBzr):
    ...
    def __init__(self, glade):
        ...
        self.scrollwindow1 = self.wTree.get_widget("appletScrollActive1")
...
class awnApplet2(awnBzr):
	...
	self.applets_list = self.type_catalog_from_sources_list(type_catalog='Applet')

        self.available_window = self.wTree.get_widget("available_applets_window")
	self.make_available_model()
        self.active_window = self.wTree.get_widget("active_applets_window")
        self.make_active_model()

        #self.load_applets()

        self.btn_deactivate = self.wTree.get_widget("remove_applet_button")
        self.btn_deactivate.connect("clicked", self.deactivate_applet)

        self.btn_activate = self.wTree.get_widget("add_applet_button")
        #self.btn_activate.connect("clicked", self.activate_applet)


    def make_active_model (self):
	''' Make the list of active applets
	'''
        self.active_model = gtk.ListStore(gtk.gdk.Pixbuf, str, str, str)
        self.active_model.connect("row-changed", self.applet_reorder)
        ...
        self.active_window.add(self.icon_view)
    ...
    def make_available_model(self):
	self.treeview_available = self.make_model(self.applets_list)
	self.available_window.add(self.treeview_available)
	self.treeview_available.enable_model_drag_dest([('text/plain', 0, 0)],
                  gdk.ACTION_DEFAULT | gdk.ACTION_MOVE)
        self.treeview_available.connect("drag_data_received", self.drag_data_received_data)
    ...
    def register_applet(self, appletpath, do_apply, applet_exists, msg=True):
        if do_apply:
            model = self.make_available_model
        else:
            model = self.make_active_model
