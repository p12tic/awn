...
task_icon_refresh_geometry(TaskIcon* icon)
{
    ...
    case GTK_POS_RIGHT:
        ...
        if (pos_type == GTK_POS_RIGHT) {
            x += alloc.width - panel_size - offset;
        }
        break;
    case GTK_POS_TOP:
    default:
        ...
        y = base_y;
        // the icon has extra space above it, this compensates it
        if (pos_type == GTK_POS_BOTTOM) {
            y += alloc.height - panel_size - offset;
        }
        break;
    }
                             ...
                             TaskIcon*  icon)
{
    ...
    if (visible && priv->main_item && !TASK_IS_LAUNCHER(priv->main_item)) {
        return;
    }
...
task_icon_count_require_attention(TaskIcon* icon)
{
    ...
    for (w = priv->items; w; w = w->next) {
        ...
        if (!task_window_get_needs_attention(TASK_WINDOW(item))) {
            continue;
        }
